...
> 🔥 **最新更新 v1.5.0**: 新增 **MCP 支持**（8 个内置服务器）、**插件系统**（运行时沙箱+SDK）、**多模态支持**（图片/音频/视频/文件/语音）！
...
**v1.5.0 新增**：Model Context Protocol 工具服务

| 服务器 | 描述 |
|--------|------|
| 📁 **filesystem** | 文件系统操作（读取/写入/搜索） |
| 📊 **git** | Git 版本控制（状态/diff/提交历史） |
| 🐙 **github** | GitHub 操作（仓库/Issues/PRs） |
| 🗄️ **sqlite** | SQLite 数据库查询 |
| 🌐 **fetch** | 网页内容获取 |
| 🌐 **puppeteer** | 浏览器自动化 |
